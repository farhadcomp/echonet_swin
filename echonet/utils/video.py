"""Functions for training and running EF prediction."""

import math
import os
import time

import click
import matplotlib.pyplot as plt
import numpy as np
import sklearn.metrics
import torch
import torch.nn as nn
import torchvision
import tqdm

import echonet

import os
import torch.distributed as dist
from torch.utils.data.distributed import DistributedSampler
from torch.nn.parallel import DistributedDataParallel as DDP

# class PhysiologyInformedLoss(nn.Module):
#     def __init__(self, lambda_phys=0.1):
#         """
#         lambda_phys: Controls how strictly we enforce the physics rule. 
#         Start small (0.1) so the model learns to guess volumes first, 
#         then you can increase it later to enforce strict physics.
#         """
#         super().__init__()
#         self.mse = nn.MSELoss()
#         self.lambda_phys = lambda_phys

#     def forward(self, predictions, targets):
#         # predictions shape: [batch_size, 3] -> [EF, EDV, ESV]
#         # targets shape:     [batch_size, 3] -> [EF, EDV, ESV]
        
#         ef_pred, edv_pred, esv_pred = predictions[:, 0], predictions[:, 1], predictions[:, 2]
#         ef_true, edv_true, esv_true = targets[:, 0], targets[:, 1], targets[:, 2]

#         # 1. Supervised Data Loss (Learn the actual values)
#         # loss_data = self.mse(ef_pred, ef_true) + \
#         #             self.mse(edv_pred, edv_true) + \
#         #             self.mse(esv_pred, esv_true)

#         weight_ef = 10.0  
#         weight_vol = 1.0

#         loss_data = (weight_ef * self.mse(ef_pred, ef_true)) + \
#                     (weight_vol * self.mse(edv_pred, edv_true)) + \
#                     (weight_vol * self.mse(esv_pred, esv_true))

#         # 2. Physiology Constraint (The "Physics" Loss)
#         # We add 1e-8 to the denominator to prevent division by zero errors
#         ef_calculated = ((edv_pred - esv_pred) / (edv_pred + 1e-8)) * 100.0
        
#         # Penalize the model if its predicted EF doesn't match its predicted volumes
#         loss_phys = self.mse(ef_pred, ef_calculated)

#         # 3. Total Loss
#         return loss_data + (self.lambda_phys * loss_phys)

class PhysiologyInformedLoss(nn.Module):
    def __init__(self, lambda_phys=0.1):
        super().__init__()
        self.mse = nn.MSELoss()
        self.lambda_phys = lambda_phys

    def forward(self, predictions, targets):
        # 1. Unpack predictions and targets
        ef_pred, edv_pred, esv_pred = predictions[:, 0], predictions[:, 1], predictions[:, 2]
        ef_true, edv_true, esv_true = targets[:, 0], targets[:, 1], targets[:, 2]

        # 2. Define biological maximums to act as our standard scaler
        MAX_EF = 100.0   # Ejection fraction is a percentage (0-100)
        MAX_VOL = 400.0  # Absolute maximum biological volume in mL

        # 3. Normalize all variables to a [0, 1] range
        # This forces the MSE to treat an EF error and a Volume error with equal respect
        loss_ef = self.mse(ef_pred / MAX_EF, ef_true / MAX_EF)
        loss_edv = self.mse(edv_pred / MAX_VOL, edv_true / MAX_VOL)
        loss_esv = self.mse(esv_pred / MAX_VOL, esv_true / MAX_VOL)

        # The data loss is now perfectly mathematically balanced natively
        loss_data = loss_ef + loss_edv + loss_esv

        # 4. Physics Constraint
        # The physics formula (EDV - ESV) / EDV is a ratio, so it naturally outputs a 0-1 scale!
        # We add 1e-8 to the denominator just to prevent accidental divide-by-zero crashes.
        physics_ef_pred = (edv_pred - esv_pred) / (edv_pred + 1e-8)
        
        # Compare the physical ratio to the predicted EF (scaled to 0-1)
        loss_phys = self.mse(physics_ef_pred, ef_pred / 100.0)

        # 5. Final combined loss
        return loss_data + (self.lambda_phys * loss_phys)


@click.command("video")
@click.option("--data_dir", type=click.Path(exists=True, file_okay=False), default=None)
@click.option("--output", type=click.Path(file_okay=False), default=None)
@click.option("--task", type=str, default="EF")
@click.option("--model_name", type=click.Choice(
    sorted(name for name in torchvision.models.video.__dict__
           if name.islower() and not name.startswith("__") and callable(torchvision.models.video.__dict__[name]))),
    default="r2plus1d_18")
@click.option("--pretrained/--random", default=True)
@click.option("--weights", type=click.Path(exists=True, dir_okay=False), default=None)
@click.option("--run_test/--skip_test", default=False)
@click.option("--num_epochs", type=int, default=45)
@click.option("--lr", type=float, default=1e-4)
@click.option("--weight_decay", type=float, default=1e-4)
@click.option("--lr_step_period", type=int, default=15)
@click.option("--frames", type=int, default=32)
@click.option("--period", type=int, default=2)
@click.option("--num_train_patients", type=int, default=None)
@click.option("--num_workers", type=int, default=4)
@click.option("--batch_size", type=int, default=20)
@click.option("--device", type=str, default=None)
@click.option("--seed", type=int, default=0)
def run(
    data_dir=None,
    output=None,
    task="EF",

    model_name="r2plus1d_18",
    pretrained=True,
    weights=None,

    run_test=False,
    num_epochs=45,
    lr=1e-4,
    weight_decay=1e-4,
    lr_step_period=15,
    frames=32,
    period=2,
    num_train_patients=None,
    num_workers=4,
    batch_size=20,
    device=None,
    seed=0,
):
    """Trains/tests EF prediction model.

    \b
    Args:
        data_dir (str, optional): Directory containing dataset. Defaults to
            `echonet.config.DATA_DIR`.
        output (str, optional): Directory to place outputs. Defaults to
            output/video/<model_name>_<pretrained/random>/.
        task (str, optional): Name of task to predict. Options are the headers
            of FileList.csv. Defaults to ``EF''.
        model_name (str, optional): Name of model. One of ``mc3_18'',
            ``r2plus1d_18'', or ``r3d_18''
            (options are torchvision.models.video.<model_name>)
            Defaults to ``r2plus1d_18''.
        pretrained (bool, optional): Whether to use pretrained weights for model
            Defaults to True.
        weights (str, optional): Path to checkpoint containing weights to
            initialize model. Defaults to None.
        run_test (bool, optional): Whether or not to run on test.
            Defaults to False.
        num_epochs (int, optional): Number of epochs during training.
            Defaults to 45.
        lr (float, optional): Learning rate for SGD
            Defaults to 1e-4.
        weight_decay (float, optional): Weight decay for SGD
            Defaults to 1e-4.
        lr_step_period (int or None, optional): Period of learning rate decay
            (learning rate is decayed by a multiplicative factor of 0.1)
            Defaults to 15.
        frames (int, optional): Number of frames to use in clip
            Defaults to 32.
        period (int, optional): Sampling period for frames
            Defaults to 2.
        n_train_patients (int or None, optional): Number of training patients
            for ablations. Defaults to all patients.
        num_workers (int, optional): Number of subprocesses to use for data
            loading. If 0, the data will be loaded in the main process.
            Defaults to 4.
        device (str or None, optional): Name of device to run on. Options from
            https://pytorch.org/docs/stable/tensor_attributes.html#torch.torch.device
            Defaults to ``cuda'' if available, and ``cpu'' otherwise.
        batch_size (int, optional): Number of samples to load per batch
            Defaults to 20.
        seed (int, optional): Seed for random number generator. Defaults to 0.
    """

    # Seed RNGs
    np.random.seed(seed)
    torch.manual_seed(seed)

    # --- ADD THIS BLOCK: DDP Initialization ---
    dist.init_process_group(backend='nccl')
    local_rank = int(os.environ["LOCAL_RANK"])
    torch.cuda.set_device(local_rank)
    device = torch.device(f"cuda:{local_rank}")
    # ------------------------------------------

    # Set default output directory
    if output is None:
        output = os.path.join("output", "video", "{}_{}_{}_{}".format(model_name, frames, period, "pretrained" if pretrained else "random"))
    os.makedirs(output, exist_ok=True)

    # Set device for computations
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Set up model
    # model = torchvision.models.video.__dict__[model_name](pretrained=pretrained)

    # # model.fc = torch.nn.Linear(model.fc.in_features, 1)
    # # model.fc.bias.data[0] = 55.6

    # # New Swin Transformer Architecture
    # model = torchvision.models.video.swin3d_s(weights='KINETICS400_V1')

    # # model.head = torch.nn.Linear(model.head.in_features, 1)
    # model.head = torch.nn.Sequential(
    #     torch.nn.Dropout(p=0.5),
    #     torch.nn.Linear(model.head.in_features, 1)
    # )
    # model.head[1].bias.data[0] = 55.6

    # if device.type == "cuda":
    #     model = torch.nn.DataParallel(model)
    # model.to(device)
    # model.to(device)
    # Wrap model with DDP. We pass the specific local GPU ID.
    # model = DDP(model, device_ids=[local_rank])

    # Set up model dynamically based on command line argument
    # if model_name == "swin3d_s":
    #     print("Initializing Custom Swin3D-S Architecture...")
    #     model = torchvision.models.video.swin3d_s(weights='KINETICS400_V1', stochastic_depth_prob=0.2)
    #     model.head = torch.nn.Sequential(
    #         torch.nn.Dropout(p=0.5),
    #         torch.nn.Linear(model.head.in_features, 1)
    #     )
    #     model.head[1].bias.data[0] = 55.6
    # else:
    #     print(f"Initializing Baseline {model_name} Architecture...")
    #     model = torchvision.models.video.__dict__[model_name](pretrained=pretrained)
    #     model.fc = torch.nn.Linear(model.fc.in_features, 1)
    #     model.fc.bias.data[0] = 55.6

    # (Physic Informed Loss)---begin
     ## Using a new model structure
    if model_name == "swin3d_s":
        print("Initializing Custom Swin3D-S Architecture (Physiology-Informed)...")
        model = torchvision.models.video.swin3d_s(weights='KINETICS400_V1')
        model.head = torch.nn.Sequential(
            torch.nn.Dropout(p=0.5),
            # CHANGE: Output 3 values instead of 1 (EF, EDV, ESV)
            torch.nn.Linear(model.head.in_features, 3)
        )
        # Set clinical priors for the biases [EF, EDV, ESV]
        model.head[1].bias.data[0] = 55.6   # Average Ejection Fraction (%)
        model.head[1].bias.data[1] = 120.0  # Average End-Diastolic Volume (mL)
        model.head[1].bias.data[2] = 50.0   # Average End-Systolic Volume (mL)
        
    else:
        print(f"Initializing Baseline {model_name} Architecture (Physiology-Informed)...")
        model = torchvision.models.video.__dict__[model_name](pretrained=pretrained)
        
        # CHANGE: Output 3 values instead of 1
        model.fc = torch.nn.Linear(model.fc.in_features, 3)
        
        # Set clinical priors for the biases [EF, EDV, ESV]
        model.fc.bias.data[0] = 55.6   # Average EF
        model.fc.bias.data[1] = 120.0  # Average EDV
        model.fc.bias.data[2] = 50.0   # Average ESV
    # (physic Informed Loss)---end

   

    model.to(device)
    # model = DDP(model, device_ids=[local_rank])

    # Physic Informed (begin)
    # Tell DDP not to wait for dropped layers!
    model = DDP(model, device_ids=[local_rank], find_unused_parameters=True)
    # Physic Informed (end)

    if weights is not None:
        checkpoint = torch.load(weights, weights_only=False, map_location="cpu")
        model.load_state_dict(checkpoint['state_dict'])

    # --- DIFFERENTIAL LEARNING RATES ---
    # print("Applying Differential Learning Rates...")    
    if local_rank == 0:
        print("Applying Differential Learning Rates...", flush=True)
    
    # 1. Make sure everything is unfrozen!
    for param in model.parameters():
        param.requires_grad = True

    # 2. Separate the weights into two groups
    base_params = []
    head_params = []
    
    for name, param in model.named_parameters():
        # Swin Transformers usually name their final layers 'head' or 'fc'
        if "head" in name or "fc" in name:
            head_params.append(param)
        else:
            base_params.append(param)
            
    print(f"Base layers parameters: {len(base_params)}")
    print(f"Classification Head parameters: {len(head_params)}")
    # -----------------------------------

    # Set up optimizer
    # optim = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay)
    # if lr_step_period is None:
    #     lr_step_period = math.inf
    # scheduler = torch.optim.lr_scheduler.StepLR(optim, lr_step_period)
    # optim = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    # optim = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, weight_decay=weight_decay)
    # The foundation layers get a tiny LR (1e-5), the head gets a larger LR (1e-4)
    optim = torch.optim.AdamW([
        {'params': base_params, 'lr': 1e-5},
        {'params': head_params, 'lr': 1e-4}
    ], weight_decay=weight_decay)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=num_epochs)

    ### This is a new block of code
    from torch.optim.lr_scheduler import LinearLR, CosineAnnealingLR, SequentialLR

    # 1. Warm-up: Start at 1% of the LR and ramp up over the first 5 epochs
    warmup_epochs = 5
    scheduler_warmup = LinearLR(optim, start_factor=0.01, end_factor=1.0, total_iters=warmup_epochs)

    # 2. Cosine Decay: Handle the remaining 40 epochs
    scheduler_cosine = CosineAnnealingLR(optim, T_max=(num_epochs - warmup_epochs))

    # 3. Chain them together! 
    scheduler = SequentialLR(optim, schedulers=[scheduler_warmup, scheduler_cosine], milestones=[warmup_epochs])

    ### End of the new block

    # Compute mean and std
    mean, std = echonet.utils.get_mean_and_std(echonet.datasets.Echo(root=data_dir, split="train"))
    # mean, std = 0.0, 1.0
    # kwargs = {"target_type": task,
    #           "mean": mean,
    #           "std": std,
    #           "length": frames,
    #           "period": period,
    #           }

    # (Physic Informed Loss)---begin
    kwargs = {"target_type": ["EF", "EDV", "ESV"],
              "mean": mean,
              "std": std,
              "length": frames,
              "period": period,
              }
    # (physic Informed Loss)---end

    # Set up datasets and dataloaders
    dataset = {}
    dataset["train"] = echonet.datasets.Echo(root=data_dir, split="train", **kwargs, pad=12)
    if num_train_patients is not None and len(dataset["train"]) > num_train_patients:
        # Subsample patients (used for ablation experiment)
        indices = np.random.choice(len(dataset["train"]), num_train_patients, replace=False)
        dataset["train"] = torch.utils.data.Subset(dataset["train"], indices)
    dataset["val"] = echonet.datasets.Echo(root=data_dir, split="val", **kwargs)

    # (Physic Informed Loss)---begin
    criterion = PhysiologyInformedLoss(lambda_phys=0.1).to(device)
    # (physic Informed Loss)---end

    # Run training and testing loops
    with open(os.path.join(output, "log.csv"), "a") as f:
        epoch_resume = 0
        bestLoss = float("inf")
        try:
            # Attempt to load checkpoint
            checkpoint = torch.load(os.path.join(output, "checkpoint.pt"), map_location="cpu", weights_only=False)
            model.load_state_dict(checkpoint['state_dict'])
            optim.load_state_dict(checkpoint['opt_dict'])
            scheduler.load_state_dict(checkpoint['scheduler_dict'])
            epoch_resume = checkpoint["epoch"] + 1
            bestLoss = checkpoint["best_loss"]
            f.write("Resuming from epoch {}\n".format(epoch_resume))
        # except FileNotFoundError:
        #     f.write("Starting run from scratch\n")

        # Physics-informed (begin)
        except FileNotFoundError:
            if local_rank == 0:
                f.write("Starting run from scratch\n")
                f.flush()
        # Physics-informed (end)
        
        # --- EARLY STOPPING SETUP ---
        patience = 7  # How many epochs to wait before giving up
        patience_counter = 0
        # ----------------------------

        # for epoch in range(epoch_resume, num_epochs):
        #     print("Epoch #{}".format(epoch), flush=True)
        #     for phase in ['train', 'val']:
        #         start_time = time.time()
        #         for i in range(torch.cuda.device_count()):
        #             torch.cuda.reset_peak_memory_stats(i)

        #         # ds = dataset[phase]
        #         # dataloader = torch.utils.data.DataLoader(
        #         #     ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"), drop_last=(phase == "train"))

        #         ds = dataset[phase]
    
        #         # Create the sampler for the training set
        #         if phase == "train":
        #             sampler = DistributedSampler(ds, shuffle=True)
        #             # Note: shuffle must be False in DataLoader when using a sampler
        #             dataloader = torch.utils.data.DataLoader(
        #                 ds, batch_size=batch_size, num_workers=num_workers, sampler=sampler, pin_memory=True, drop_last=True)
        #         else:
        #             # Validation usually doesn't need a distributed sampler if you just want to evaluate on GPU 0, 
        #             # but for simplicity, standard dataloader is fine here.
        #             dataloader = torch.utils.data.DataLoader(
        #                 ds, batch_size=batch_size, num_workers=num_workers, shuffle=False, pin_memory=True)

        #         loss, yhat, y = echonet.utils.video.run_epoch(model, dataloader, phase == "train", optim, device)
        #         f.write("{},{},{},{},{},{},{},{},{}\n".format(epoch,
        #                                                       phase,
        #                                                       loss,
        #                                                       sklearn.metrics.r2_score(y, yhat),
        #                                                       time.time() - start_time,
        #                                                       y.size,
        #                                                       sum(torch.cuda.max_memory_allocated() for i in range(torch.cuda.device_count())),
        #                                                       sum(torch.cuda.max_memory_reserved() for i in range(torch.cuda.device_count())),
        #                                                       batch_size))
        #         f.flush()
        #     scheduler.step()
        for epoch in range(epoch_resume, num_epochs):
            
            # --- DYNAMIC WEIGHT DECAY (Run by ALL GPUs) ---
            lambda_start = 0.10   
            lambda_end = 0.005    
            
            # Calculate Cosine Annealing decay
            current_lambda = lambda_end + 0.5 * (lambda_start - lambda_end) * \
                             (1 + math.cos(math.pi * epoch / num_epochs))
            
            # Update the living criterion object
            criterion.lambda_phys = current_lambda
            # ----------------------------------------------

            # 1. ISOLATE PRINTING: Only the master GPU prints to the terminal
            if local_rank == 0:
                print(f"Epoch #{epoch} | Physics Weight: {current_lambda:.4f}", flush=True)
                
            for phase in ['train', 'val']:
                start_time = time.time()
                
                # 2. ISOLATE MEMORY: Only reset stats for this specific process's GPU
                torch.cuda.reset_peak_memory_stats(local_rank)

                ds = dataset[phase]
                
                # 3. DDP SAMPLER & DATALOADER SETUP
                if phase == "train":
                    # Chunk the data so each of the 8 GPUs gets a unique subset
                    sampler = DistributedSampler(ds, shuffle=True)
                    # Critical: Tell the sampler the epoch number so it shuffles differently each time
                    sampler.set_epoch(epoch)
                    
                    dataloader = torch.utils.data.DataLoader(
                        ds, batch_size=batch_size, num_workers=num_workers, 
                        sampler=sampler, pin_memory=True, drop_last=True
                    )
                else:
                    # For validation, we let all GPUs run the standard evaluation
                    dataloader = torch.utils.data.DataLoader(
                        ds, batch_size=batch_size, num_workers=num_workers, 
                        shuffle=False, pin_memory=True, drop_last=False
                    )

                # Run the epoch (No changes needed here)
                # loss, yhat, y = echonet.utils.video.run_epoch(model, dataloader, phase == "train", optim, device)

                # (Physic Informed Loss)---begin
                loss, yhat, y = run_epoch(model, dataloader, phase == "train", optim, device, criterion)
                # (physic Informed Loss)---end


                
                
                # 4. ISOLATE LOGGING: Only GPU 0 writes to the log.csv to prevent file corruption
                if local_rank == 0:
                    f.write("{},{},{},{},{},{},{},{},{}\n".format(epoch,
                                                                  phase,
                                                                  loss,
                                                                  sklearn.metrics.r2_score(y, yhat),
                                                                  time.time() - start_time,
                                                                  y.size,
                                                                  torch.cuda.max_memory_allocated(local_rank),
                                                                  torch.cuda.max_memory_reserved(local_rank),
                                                                  batch_size))
                    f.flush()

            # Save checkpoint
            save = {
                'epoch': epoch,
                'state_dict': model.state_dict(),
                'period': period,
                'frames': frames,
                'best_loss': bestLoss,
                'loss': loss,
                'r2': sklearn.metrics.r2_score(y, yhat),
                'opt_dict': optim.state_dict(),
                'scheduler_dict': scheduler.state_dict(),
            }
            torch.save(save, os.path.join(output, "checkpoint.pt"))
            # if loss < bestLoss:
            #     torch.save(save, os.path.join(output, "best.pt"))
            #     bestLoss = loss
            # --- EARLY STOPPING LOGIC ---
            if loss < bestLoss:
                torch.save(save, os.path.join(output, "best.pt"))
                bestLoss = loss
                patience_counter = 0  # Reset counter because model improved!
            else:
                patience_counter += 1
                print("Early stopping counter: {} out of {}".format(patience_counter, patience), flush=True)
                
                if patience_counter >= patience:
                    print("Early stopping triggered! Model has stopped improving.", flush=True)
                    break # Instantly kills the epoch loop
            # ----------------------------

        # Load best weights
        if num_epochs != 0:
            # checkpoint = torch.load(os.path.join(output, "best.pt"))
            checkpoint = torch.load(os.path.join(output, "best.pt"), map_location="cpu", weights_only=False)
            model.load_state_dict(checkpoint['state_dict'])
            f.write("Best validation loss {} from epoch {}\n".format(checkpoint["loss"], checkpoint["epoch"]))
            f.flush()

        if run_test:
            for split in ["val", "test"]:
                # Performance without test-time augmentation
                dataloader = torch.utils.data.DataLoader(
                    echonet.datasets.Echo(root=data_dir, split=split, **kwargs),
                    batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"))
                # loss, yhat, y = echonet.utils.video.run_epoch(model, dataloader, False, None, device)
                
                # (Physic Informed Loss)---begin
                loss, yhat, y = run_epoch(model, dataloader, False, None, device, criterion)
                # (physic Informed Loss)---end
                f.write("{} (one clip) R2:   {:.3f} ({:.3f} - {:.3f})\n".format(split, *echonet.utils.bootstrap(y, yhat, sklearn.metrics.r2_score)))
                f.write("{} (one clip) MAE:  {:.2f} ({:.2f} - {:.2f})\n".format(split, *echonet.utils.bootstrap(y, yhat, sklearn.metrics.mean_absolute_error)))
                f.write("{} (one clip) RMSE: {:.2f} ({:.2f} - {:.2f})\n".format(split, *tuple(map(math.sqrt, echonet.utils.bootstrap(y, yhat, sklearn.metrics.mean_squared_error)))))
                f.flush()

                # Performance with test-time augmentation
                ds = echonet.datasets.Echo(root=data_dir, split=split, **kwargs, clips="all")
                dataloader = torch.utils.data.DataLoader(
                    ds, batch_size=1, num_workers=num_workers, shuffle=False, pin_memory=(device.type == "cuda"))
                # loss, yhat, y = echonet.utils.video.run_epoch(model, dataloader, False, None, device, save_all=True, block_size=batch_size)
                # (Physic Informed Loss)---begin
                loss, yhat, y = run_epoch(model, dataloader, False, None, device, criterion, save_all=True, block_size=batch_size)
                # (physic Informed Loss)---end
                f.write("{} (all clips) R2:   {:.3f} ({:.3f} - {:.3f})\n".format(split, *echonet.utils.bootstrap(y, np.array(list(map(lambda x: x.mean(), yhat))), sklearn.metrics.r2_score)))
                f.write("{} (all clips) MAE:  {:.2f} ({:.2f} - {:.2f})\n".format(split, *echonet.utils.bootstrap(y, np.array(list(map(lambda x: x.mean(), yhat))), sklearn.metrics.mean_absolute_error)))
                f.write("{} (all clips) RMSE: {:.2f} ({:.2f} - {:.2f})\n".format(split, *tuple(map(math.sqrt, echonet.utils.bootstrap(y, np.array(list(map(lambda x: x.mean(), yhat))), sklearn.metrics.mean_squared_error)))))
                f.flush()

                # Write full performance to file
                with open(os.path.join(output, "{}_predictions.csv".format(split)), "w") as g:
                    for (filename, pred) in zip(ds.fnames, yhat):
                        for (i, p) in enumerate(pred):
                            g.write("{},{},{:.4f}\n".format(filename, i, p))
                echonet.utils.latexify()
                yhat = np.array(list(map(lambda x: x.mean(), yhat)))

                # Plot actual and predicted EF
                fig = plt.figure(figsize=(3, 3))
                lower = min(y.min(), yhat.min())
                upper = max(y.max(), yhat.max())
                plt.scatter(y, yhat, color="k", s=1, edgecolor=None, zorder=2)
                plt.plot([0, 100], [0, 100], linewidth=1, zorder=3)
                plt.axis([lower - 3, upper + 3, lower - 3, upper + 3])
                plt.gca().set_aspect("equal", "box")
                plt.xlabel("Actual EF (%)")
                plt.ylabel("Predicted EF (%)")
                plt.xticks([10, 20, 30, 40, 50, 60, 70, 80])
                plt.yticks([10, 20, 30, 40, 50, 60, 70, 80])
                plt.grid(color="gainsboro", linestyle="--", linewidth=1, zorder=1)
                plt.tight_layout()
                plt.savefig(os.path.join(output, "{}_scatter.pdf".format(split)))
                plt.close(fig)

                # Plot AUROC
                fig = plt.figure(figsize=(3, 3))
                plt.plot([0, 1], [0, 1], linewidth=1, color="k", linestyle="--")
                for thresh in [35, 40, 45, 50]:
                    fpr, tpr, _ = sklearn.metrics.roc_curve(y > thresh, yhat)
                    print(thresh, sklearn.metrics.roc_auc_score(y > thresh, yhat))
                    plt.plot(fpr, tpr)

                plt.axis([-0.01, 1.01, -0.01, 1.01])
                plt.xlabel("False Positive Rate")
                plt.ylabel("True Positive Rate")
                plt.tight_layout()
                plt.savefig(os.path.join(output, "{}_roc.pdf".format(split)))
                plt.close(fig)

def run_epoch(model, dataloader, train, optim, device, criterion, save_all=False, block_size=None):
# def run_epoch(model, dataloader, train, optim, device, save_all=False, block_size=None):
    """Run one epoch of training/evaluation for segmentation.

    Args:
        model (torch.nn.Module): Model to train/evaulate.
        dataloder (torch.utils.data.DataLoader): Dataloader for dataset.
        train (bool): Whether or not to train model.
        optim (torch.optim.Optimizer): Optimizer
        device (torch.device): Device to run on
        save_all (bool, optional): If True, return predictions for all
            test-time augmentations separately. If False, return only
            the mean prediction.
            Defaults to False.
        block_size (int or None, optional): Maximum number of augmentations
            to run on at the same time. Use to limit the amount of memory
            used. If None, always run on all augmentations simultaneously.
            Default is None.
    """

    model.train(train)

    total = 0  # total training loss
    n = 0      # number of videos processed
    s1 = 0     # sum of ground truth EF
    s2 = 0     # Sum of ground truth EF squared

    yhat = []
    y = []

    # with torch.set_grad_enabled(train):
    #     with tqdm.tqdm(total=len(dataloader)) as pbar:
    #         for (X, outcome) in dataloader:

    # Physics Infomed (bigin)
    with torch.set_grad_enabled(train):
        # 1. Grab the local rank
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
        
        # 2. Disable the progress bar for all GPUs except GPU 0
        with tqdm.tqdm(total=len(dataloader), disable=(local_rank != 0)) as pbar:
            for (X, outcome) in dataloader:
    # Physics Informed (end)

                # y.append(outcome.numpy())

                # two line added to stack list
                if isinstance(outcome, (list, tuple)):
                    outcome = torch.stack(outcome, dim=1).float()

                # (Physic Informed Loss)---begin
                y.append(outcome[:, 0].numpy())
                # (physic Informed Loss)---end
                
                X = X.to(device)
                outcome = outcome.to(device)

                average = (len(X.shape) == 6)
                if average:
                    batch, n_clips, c, f, h, w = X.shape
                    X = X.view(-1, c, f, h, w)

                # Add to fix swin transformer dimensions
                X = torch.nn.functional.interpolate(X, size=(X.shape[2], 224, 224), mode='trilinear', align_corners=False)

                # s1 += outcome.sum()
                # s2 += (outcome ** 2).sum()

                # (Physic Informed Loss)---begin
                s1 += outcome[:, 0].sum().item()
                s2 += (outcome[:, 0] ** 2).sum().item()
                # (physic Informed Loss)---end

                if block_size is None:
                    outputs = model(X)
                else:
                    outputs = torch.cat([model(X[j:(j + block_size), ...]) for j in range(0, X.shape[0], block_size)])

                # if save_all:
                #     yhat.append(outputs.view(-1).to("cpu").detach().numpy())

                # if average:
                #     outputs = outputs.view(batch, n_clips, -1).mean(1)

                # if not save_all:
                #     yhat.append(outputs.view(-1).to("cpu").detach().numpy())

                # (Physic Informed Loss)---begin
                if save_all:
                    yhat.append(outputs[:, 0].to("cpu").detach().numpy())

                if average:
                    outputs = outputs.view(batch, n_clips, -1).mean(1)

                if not save_all:
                    yhat.append(outputs[:, 0].to("cpu").detach().numpy())
                # (physic Informed Loss)---end  

                # loss = torch.nn.functional.mse_loss(outputs.view(-1), outcome)
                # loss = torch.nn.functional.smooth_l1_loss(outputs.view(-1), outcome, beta=1.0)

                # (Physic Informed Loss)---begin
                loss = criterion(outputs, outcome)
                # (physic Informed Loss)---end                

                if train:
                    optim.zero_grad()
                    loss.backward()
                    optim.step()

                total += loss.item() * X.size(0)
                n += X.size(0)

                pbar.set_postfix_str("{:.2f} ({:.2f}) / {:.2f}".format(total / n, loss.item(), s2 / n - (s1 / n) ** 2))
                pbar.update()

    if not save_all:
        yhat = np.concatenate(yhat)
    y = np.concatenate(y)

    return total / n, yhat, y

if __name__ == '__main__':
    run()
