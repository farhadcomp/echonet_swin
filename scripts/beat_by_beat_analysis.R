...
data <- read.csv("/home/AD.UNLV.EDU/farhadik/tests/dynamic/output/video/r2plus1d_18_32_2_pretrained/test_predictions.csv", header = FALSE)
str(data)
...
sizeData <- read.csv("/home/AD.UNLV.EDU/farhadik/tests/dynamic/output/segmentation/deeplabv3_resnet50_random/size.csv")
...
fileLocation <- "/home/AD.UNLV.EDU/farhadik/echo/"
# ActualNumbers <- read.csv(paste0(fileLocation, "FileList.csv", sep = ""))
ActualNumbers <- read.csv(paste0(fileLocation, "FileList.csv"), colClasses = c("FileName" = "character"))
ActualNumbers <- ActualNumbers[c(1,2)]
names(ActualNumbers)[1] <- "Filename"
ActualNumbers$Filename <- paste0(ActualNumbers$Filename, ".avi")
str(ActualNumbers)
...
ggsave("beat_by_beat_boxplot.pdf", plot = beatBoxPlot, width = 8, height = 6)
print("Analysis complete! Plot saved as beat_by_beat_boxplot.pdf")
